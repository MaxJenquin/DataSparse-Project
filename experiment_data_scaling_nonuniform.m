%% Numerical Experiment: fixed dimensionality, nonuniform grid, dataset scaling
...
% SKI) for a kernel with axis-multiplicative structure as the size of input
% data grows, for a fixed-dimension problem (here d=3).

% This experiment is complementary to the experiment "fixed dataset scale,
% nonuniform grid, dimensionality scaling", which simply scales problem
...
% located in "experiment_dim_scaling_nonuniform.m"
...
% each subdomain will have n grid locations per axis, where n is in 
% [half_data_low*data_scale, half_data_high*data_scale]
...
for dscale_ind=1:num_dscales
    ...
    size_counter = [0,0,0];
    for axis=1:num_dims
        upper = round(half_data_high*data_scale);
        lower = round(half_data_low*data_scale);
        foo = randsample(lower:upper,1);
        bar = randsample(lower:upper,1);
...
save('data/data_scaling_nonuniform.mat','default_times','rskelf_times',...
